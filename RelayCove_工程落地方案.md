...
采用“自绘标题栏 + Navigation Rail + Conversation Panel + Chat Panel + 条件式成员抽屉”的桌面聊天布局。Rail 承载账户、聊天、频道、设置等低频入口；Conversation Panel 承载公开频道、私有频道和私聊；Chat Panel 始终使用剩余宽度；成员抽屉只在宽窗口按需打开，窄窗口保持关闭，不得挤压聊天与输入区。

```text
┌──────────────────────────────────────────────────────────────────┐
│ 自绘标题栏：窗口命令、产品标题与系统菜单                            │
├───────┬────────────────┬────────────────────────────┬───────────┤
│ Rail  │ Conversation   │ Chat Panel                 │ 成员抽屉  │
│       │ 搜索/筛选/分组  │ Header / 消息列表 / 输入区   │ 条件打开  │
│ 低频  │ 公开/私有/私聊  │                            │ >=1400px │
│ 入口  │ 未读与选择态    │                            │           │
└───────┴────────────────┴────────────────────────────┴───────────┘
```

窗口最小 900×520；标题栏固定 48px 并使用 WPF `WindowChrome` 保留 DPI、边缘缩放和最大化工作区。宽度 `>=1400` 时 Rail 72px、会话栏 340px、成员抽屉可打开且为 360px；`1100–1399` 时为 72px/320px 且抽屉关闭；`900–1099` 时为 64px/280px 且抽屉关闭。关闭按钮必须继续进入现有关闭到托盘流程，真正退出和更新交接不得由 UI 重构绕过。
...
v1 登录名限制为 2–64 个 ASCII 字母、数字、点、下划线或连字符；`UserName` 保留原始大小写，所有写入通过同一实体方法同步生成 invariant-uppercase `NormalizedUserName`，登录查找和唯一约束只使用后者。Unicode 姓名使用 `DisplayName`，不把 ICU/NLS 相关大小写规则带入身份键。
...
只有当前正常管理员账号可以登录。Windows 客户端不提供全局管理员入口；管理员在
对应服务器的网页后台完成账号、全局频道、附件上限和服务器状态维护。客户端保留
日常聊天，以及所有正常用户可用的频道创建和私有频道成员管理。
...
```text
实现由 ASP.NET Core Server 提供的基础网页管理，不在 Windows 聊天客户端提供全局管理 UI。
```

任务：

1. 管理员网页入口（`https://<server>/<path-base>/admin/`）；
2. 创建账号；
3. 禁用账号；
4. 删除账号；
5. 重置密码；
