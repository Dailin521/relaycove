...
- 本地缓存根目录可在存储设置指定，包含全部账号数据库、表情及收藏、更新包和头像；选择目录后下次启动迁移，必须先取得单实例且在任何数据库/缓存服务构造前完成。复制含 SQLite WAL 的完整账号目录，逐文件校验、原子提交路径后才删除已验证源文件。配置/凭据留在固定系统目录；自定义路径不可用不得静默回退为新的默认空库。禁止目录联接、网络盘、源目标重叠或覆盖其他数据；普通附件下载位置独立。
...
更细的现有功能与交互见 `docs/FEATURES.md`；代码和当前运行结果优先于旧文档措辞。

### 表情包交互约定

输入区表情面板依次提供“表情”“gif”“收藏”三页，每次打开回到第一项。“表情”保持组织小表情插入光标位置；“gif”（原搜索表情）/收藏中的大图独立上传、独立发送，不消费文字和附件草稿。准备或上传时切换账号/会话取消发送；消息请求已发出后的未知结果不自动重试。表情不会混入消息 reaction 选择器。

收藏成功不在聊天区显示提示卡片。

公开搜索使用 ChineseBQB 目录的名称索引，不提供图片文字识别或分类选择入口；空关键词可浏览，每批追加 60 张，首次滚动超过 90% 时触发下一批并保持当前偏移。独立无凭据客户端只允许同一 HTTPS 来源的目录与媒体路径；目录最多缓存 24 小时、媒体缓存预算 200 MiB、最多四路图片下载。收藏为按账号隔离的本机原始副本，按内容哈希去重，不参与缓存淘汰，不在注销时删除。支持 PNG/JPEG/WebP/GIF，单张上限 25 MiB；发送同时遵守 Realm 上传限制。聊天图片收藏通过已有受保护媒体会话读取，不能转交第三方客户端。GIF 不转静态图，面板悬停预览，卸载时停止播放。

## 7. 验证与发布

客户端默认在每次完整启动后异步检查 GitHub `Dailin521/relaycove` 的正式 Release，不依赖 Realm 登录。每个新构建只在窗口可见时提醒一次，可在账户设置关闭，关于页保留手动检查、下载和打开安装包入口；下载完成不自动安装或重启。
...
安装器安装前按目标目录的完整 `RichChat.exe` 路径核实进程。用户明确确认后，使用同一进程句柄结束旧版本并最多等待 5 秒；默认否、拒绝关闭、权限不足、身份无法确认或退出超时均阻止继续，不按名称全局终止其他程序。静默模式要求显式 `/CLOSEAPPLICATIONS`，`/NOCLOSEAPPLICATIONS` 优先；Restart Manager 的 `CloseApplications=force` 仍保留。新版应用将 Restart Manager 正式 `WM_ENDSESSION` 确认接入真实退出流程，不在询问或取消阶段退出。安装后不自动重启，升级前先处理未保存内容；这部分自 1.0.8 起生效。修改此策略时运行 `scripts/test-installer-close.ps1 -IsccPath <ISCC路径>`，并用 `-RejectShutdownQuery` 复验拒绝关闭场景；测试仅操作隔离文件和进程，不操作用户应用。
