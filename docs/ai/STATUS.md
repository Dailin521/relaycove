...
- `agent/stage-7-desktop-attention-tray`：实现 Toast 成功后的单次提示音/任务栏闪烁、NotifyIcon 状态与 Open/Exit，以及普通关闭隐藏/彻底退出生命周期；真实账户状态接线留给阶段 8。
...
完成提示音、任务栏闪烁与托盘/关闭隐藏生命周期，再进入阶段 8 账户 runtime 与最小 UI 组合。
