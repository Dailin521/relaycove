...
- stage-19 rc.24 更新交付与 Windows 人工验收：`进行中`。S4 已完成；仅在取得 SHA-256 匹配的 rc.23 ZIP 后运行更新交付演练，并在具备两个测试账户和通知环境后执行人工矩阵
...
owner 提供精确 rc.23 ZIP 后执行更新交付演练；随后在真实 Windows 双账号和通知环境执行人工矩阵。公开 Tag/Release、线上更新通道写入和严格第二 Windows UI 矩阵仍需单独授权或决策。
