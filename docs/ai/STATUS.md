...
Updated: 2026-08-17
Branch: `codex/stage-24-2-private-chat-switching` (isolated worktree `E:\WorkSpace\RelayCove-Stage22MParity`)
Integration commits: Web `573a33d`; first MAUI slice `c1c4da9`; native Web-parity follow-up `259e176`
Current delivery: Stage 22W/22M and Stage 23 remain preserved in history. Stage 24 and its cache-first/manual-incident follow-up were merged to `origin/main` through PR #1 (`537b1cd`) and PR #2 (`67fcab4`). A second private-chat switching follow-up is local on `codex/stage-24-2-private-chat-switching`; it is uncommitted, unpushed and undeployed, and has not exercised the Realm. Stage 23's isolated three-scenario Live gate remains historical evidence and was not rerun for Stage 24.

## Stage 24 native product polish — current local evidence, 2026-08-17

- The 2026-08-17 private-chat switching follow-up projects an already-cached Core window synchronously in the click path, suppresses equivalent SQLite/network page publications and coalesces a second click on the same still-pending DM. A memory/SQLite hit now requests the real bottom immediately; authoritative revalidation only replaces that request when it contributes a genuinely newer message ID. Programmatic realization/scroll callbacks are excluded from automatic older-page loading and prepend-anchor sampling; once the target container exists, WinUI performs a no-animation native bottom change and verifies it on later layout frames.
...
- Conversation activation publishes a structured conversation/generation/target/reason scroll request as soon as the current generation has a cached target, while keeping the activation intent through authoritative revalidation. The native view waits for item, handler, extent and target layout, verifies the bottom before acknowledgement, and lets current-generation explicit bottom requests supersede prepend anchors.
...
- Direct-message navigation rows and keyed message presentation rows now keep stable observable instances and update changed fields in place. A 12-conversation App presentation LRU complements Core's bounded history windows, and avatar cache hits reuse the same in-memory `ImageSource`, preventing selection/read-state changes from recreating avatar controls or decoded sources.
...
- The 2026-08-17 follow-up ran no automated tests, Fast, Full or Live. A Debug-only, network-disabled `dm-cache-switch` scene seeded 120 messages in each of two DMs and executed A -> B -> A through the production navigation path. Secondary-display PrintWindow captures at 1440, 1024 and 640 DIP all settled on message 120; the tall final message remained fully above Composer. Two delayed 640 captures were pixel-identical, and the tracked process remained responsive. This is bounded final-state evidence, not real-input, Realm, network-latency or frame-by-frame flicker acceptance.
- After the native-bottom arbitration follow-up, one exact Rebuild could not replace output DLLs because the user-run `RelayCove.App` PID 34276 and Visual Studio PID 13928 still held them (`MSB3061`, `MSB3026`, `MSB3027`, `MSB3021`; 89 warnings/6 errors). After the app was closed, the final stable-row/cache-first exact Rebuild passed with 0 warnings and 0 errors in 7.56 seconds.
- The 1024-DIP preview exposed a layout-reflow regression after the initial bottom request had already been acknowledged: viewport/extent changes could leave the latest item behind Composer. The Windows view now retains bottom only when the pre-change real distance was <=96 DIP, without stealing a user viewport farther away. The final exact App Debug Rebuild passed with 0 warnings and 0 errors in 7.07 seconds.
...
- Not run for Stage 24: Fast, Full, Live, real credentials/Realm traffic, formal Windows Machine manual interaction, the complete screenshot matrix, 100%/200%, high contrast, package installation or clean Windows 11 VM. The bounded Debug preview captures above do not close these gates; no current package hash is claimed.
