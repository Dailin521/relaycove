...
Updated: 2026-08-25
Integration basis: Stage 31 delivery `main@068b1d1`; Stage 30 functional delivery `main@57c8145`; current V2 planning baseline `main@6e8d0e9`; first MAUI release candidate and strict Release-XamlC corrections `v2.0.0-alpha.1@83eccf0`
Integration commits: Web `573a33d`; first MAUI slice `c1c4da9`; native Web-parity follow-up `259e176`
Current delivery: the first MAUI development cycle through Stage 30 is complete. V2 has an approved MAUI-only optimization plan. Stage 31 Windows message-body selection was confirmed and delivered as `main@068b1d1`. Stage 32 restores the existing server-side saved-message list and adds exact message-anchor navigation; its entry was confirmed visible, while the corrected anchor still awaits final user confirmation. Stage 33 reaction-button clipping is confirmed fixed. Stage 34 gives the reaction picker its own quick-action anchor and still awaits Visual Studio confirmation. Stage 35 reduces image messages to their preview frame, removes the outer bubble for image-only messages and adds an image-specific download action to the normal right-click message menu; the user confirmed the final behavior in Visual Studio. `v2.0.0-alpha.1` was published from `83eccf04a114defc9fc479f447d30709fb2d3807` as the Latest Windows 11 x64 GitHub Release. It is not a draft or prerelease; signing, installer and clean-VM acceptance remain open. Stage 25 alone performed authorized production Realm writes to archive every existing active channel; Stage 29 used only the public static Zulip 12.1 catalog, and Stages 30–35 performed no authenticated read or Realm write. Stage 23's isolated three-scenario Live gate remains historical evidence and was not rerun.

## Stage 35 image preview and context download — 2026-08-25

- The image and ordinary-file presentation previously shared one unconditional metadata row. Image messages therefore showed the controlled preview plus a permanent file icon, name, kind and Download button. That row is now restricted to ordinary files. The first Visual Studio check then exposed the remaining blue outer bubble; messages whose presentation contains only images now also use a transparent, borderless, zero-padding bubble, while mixed text/image and ordinary-file messages retain the normal bubble.
- A Windows image-preview right-click behavior now resolves both the virtualized message and the exact clicked attachment. It opens the existing normal message menu at the pointer and conditionally adds “下载原图”; right-clicking other message content does not show that item. Left-click image viewing and ordinary file cards remain unchanged.
- Focused presentation/menu/download tests passed 4/4, and the image-only bubble regressions passed 2/2. The isolated App Debug build after the follow-up passed with 0 warnings/errors. The user confirmed the final image presentation and interaction in Visual Studio. Complete App tests, Fast, Full, Live, package, Agent app startup, screenshot, Realm access and external writes were not run.

## Stage 34 reaction-picker anchor — 2026-08-25

- The reaction picker was already hosted by the shared popover-positioning behavior, but its XAML read message-menu coordinates. Quick smile actions never populated those coordinates, leaving `(0,0)` and placing the panel at the main-content top-left.
- Quick actions now pass a dedicated reaction-picker request derived from the native trigger's current page coordinates. The preferred 310-DIP horizontal position is constrained to the message-list content column before the existing behavior applies vertical flip and final window-edge clamping. Message-menu, Composer picker and reaction protocol behavior are unchanged.
- Focused anchor/layout tests passed 4/4 after correcting a test fixture that lacked its register-like recent-DM projection. The isolated App Debug build passed with 0 warnings/errors. User Visual Studio confirmation is pending; complete App tests, Fast, Full, Live, package, Agent app startup, screenshot, Realm access and external writes were not run.

## Stage 33 reaction-button clipping — 2026-08-25

- User captures showed every reaction button losing only its right rounded edge while the emoji, count and parent bubble had enough width. The issue was not message-content truncation: native Buttons were direct wrapping FlexLayout children and also carried their own trailing/bottom margin, so the Windows child layout slot clipped the right-side background and stroke.
- Spacing now belongs to a lightweight measured wrapper, while each native Button has zero margin. Content-sized width, wrapping, selected state, click/keyboard behavior and server reaction identity remain unchanged; no fixed reaction or bubble width was introduced.
- The focused reaction and short-bubble layout regressions passed 2/2. The isolated App Debug build passed with 0 warnings/errors. The user confirmed the corrected reaction rendering in Visual Studio. Complete App tests, Fast, Full, Live, package, Agent app startup, screenshot, Realm access and external writes were not run.

## Stage 32 saved-messages entry — 2026-08-25

- The account menu now exposes “收藏的消息”. It reuses the existing server-starred load, pagination, precise message opening and unstar commands instead of creating a second local saved-message model.
- The saved list occupies the existing right content column while the unified conversation list remains visible on wide/compact windows; narrow layout switches directly to the saved content. A visible return action restores the current chat. The old primary navigation rail, contacts entry and third column remain absent.
- Focused App evidence passed 12/12, covering shell structure, account-menu routing, narrow workspace state, pagination-anchor handling and unsupported-conversation filtering. The isolated App Debug build passed with 0 warnings/errors.
- The first Visual Studio check confirmed the entry but exposed that opening a saved message landed at the bottom of its around page. The server already loaded around the exact ID; App had classified the projection as an ordinary conversation activation and replaced the anchor with the page's maximum ID. A dedicated `MessageAnchor` request now preserves the saved message ID, centers that row without forcing latest, and does not mark the viewport as bottom. The focused anchor regression passed 3/3 and the corrected App Debug build passed with 0 warnings/errors.
- User Visual Studio confirmation is pending. Complete App tests, Fast, Full, Live, package, Agent app startup, screenshot, Realm access and external writes were not run.
...
- The final self-contained ZIP is 96,496,867 bytes with SHA-256 `8ED67737368D6C14FE7168F3E9A34DEB0E403EA8DE82DB47ED86F6005C8DCAD3`. Its ProductVersion identifies the same full `83eccf04a114defc9fc479f447d30709fb2d3807` revision as the annotated release tag.
