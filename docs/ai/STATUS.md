...
- **当前阶段：** stage-19 进行中 — rc.24 S0–S4 已通过；owner 已决定跳过 S5（保持未验证）；S6 需要真实 Windows 双账号与通知环境，当前测试宿主的原生通知注册也不可用
...
- stage-19 Windows 人工验收：`进行中`。S4 已完成；owner 已决定跳过 S5 且不将其计为通过，待具备两个测试账户和通知环境后执行人工矩阵
...
在真实 Windows 双账号和通知环境执行人工矩阵；rc.23→rc.24 更新演练经 owner 决定跳过且保持未验证。公开 Tag/Release、线上更新通道写入和严格第二 Windows UI 矩阵仍需单独授权或决策。
