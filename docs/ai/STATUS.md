...
- **当前阶段：** stage-20 rc.25 UI 重构正在完成 S22 线上反馈修复：Header 标题收敛、私聊全员目录、紧凑会话栏与发送后滚动稳定性；待从最终已提交 HEAD 完成 Fast/Full、双构建包校验及独立只读复核
...
- **最近验证通过的状态：** S22 复核修正后的 Fast/Full 通过：Shared 70、Server 353、Client 1,272、Updater 38，合计 1,733 项；Debug/Release 均为 0 警告、0 错误，Full 同时确认格式与 `git diff --check`。
...
- 待提交 S22 主界面、目录与滚动稳定性修复后，从精确 HEAD 生成两份独立 rc.25 Windows x64 自包含包并进行离线校验。真实 Windows 100%/125%/150% DPI、拖动/边缘缩放/Alt+Space、托盘恢复/真正退出和强制更新交接均保持 `未验证`；不发布更新通道
