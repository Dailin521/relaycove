...
- **当前阶段：** stage-20 rc.25 UI 重构已完成 S0–S19 代码、组件、视觉与交互收口；待从最终已提交 HEAD 完成双构建包校验及独立只读复核
...
- **最近验证通过的状态：** S19 Fast/Full 通过：Shared 70、Server 353、Client 1,266、Updater 38，合计 1,727 项，Debug/Release 均为 0 警告、0 错误；Full 同时确认 format 与 `git diff --check`。快照见 `artifacts/rc25/ui-snapshots/after-s19-chat-polish/`。
...
- 待提交 S19 聊天体验收口后，从精确 HEAD 生成两份独立 rc.25 Windows x64 自包含包并进行离线校验。真实 Windows 100%/125%/150% DPI、拖动/边缘缩放/Alt+Space、托盘恢复/真正退出和强制更新交接均保持 `未验证`；不发布更新通道
