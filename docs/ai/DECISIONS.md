...
### DEC-049：附件下载 UI 以精确 flight 状态提交并只暴露受控目录定位

- **状态：** 已接受
- **日期：** 2026-08-04
- **背景：** `DEC-048` 已冻结可信下载与账户隔离 cache，但 WPF 尚未展示 confirmed 附件、持久下载结果或运行中状态。recycling virtualization、selection refresh 和 A→B→A 会让按 ID 或表面值比较的迟到 progress/result 写入新行；把 `LocalPath` 或任意路径交给 UI/shell，又会绕过当前授权、物理完整性与撤权清理。opaque `.cache` 没有可信扩展名，直接依赖系统文件关联或复制成原名还会引入执行、MOTW 与失控副本边界。
- **决策：** confirmed Image/File 按 DTO 顺序投影安全文件名、格式化大小和同一授权事务读取的 Downloaded ID 集；Text/System/pending 不产生下载动作。每个附件状态绑定 exact runtime、Ready selection、消息/附件 membership、context version、presentation state object 和不可复用 flight object；进度只按 10% bucket 单调发布。用户只能取消自己创建的 flight，旧 progress/result、refresh、切换、撤权、注销和退出均不能提交到当前行。
- **决策：** 第一检查点只开放“在文件夹中显示”。WPF 只向 runtime 提交 attachment identity；内部 presentation/state 可保留 exact identity 用于成员与 flight 门，但可渲染字段、`ToString()`、自动化名称和日志均不暴露本地路径、URL、hash 或 ID。cache 在账户 operation gate 内重新读取 DB，验证严格托管相对路径与 scope、非 reparse、长度和完整 SHA-256，并保持 `FileShare.Read` 的只读 handle 固定内容；随后在短 immediate SQLite 事务内复核完整附件记录、Downloaded 状态与当前会话授权，生成只供 Windows native shell 使用且调试投影脱敏的内部 capability；native shell boundary 不接受上层提交的任意路径或原始 ID。
- **决策：** reveal 的提交点是仍持有状态门时对当前 runtime、Ready selection、exact membership 与取消状态的最终确认并返回 `Revealed` 授权。一旦提交，释放全部锁后必须执行对应 shell start；随后发生的撤权、selection 变化或 runtime 终止按线性化顺序位于提交之后，不能抑制该已授权效果。提交前发生的撤权、selection/DB 变化或 runtime 终止必须零 shell，已启动的外部 Explorer 不可召回。调用 native shell 前必须释放 state gate、cache operation gate、commit gate 和 SQLite transaction；runtime 只跟踪到提交完成，不等待不受控的 Explorer 生命周期，而 pinned handle 保持到 native 调用返回以阻止内容替换并让 purge 延后重试。
- **决策：** Windows 实现显式调用 `CoInitializeEx`，接受已由其他 apartment 初始化的 `RPC_E_CHANGED_MODE`，通过 `SHParseDisplayName` 获取完整 PIDL，并使用 Microsoft 明确支持的 `SHOpenFolderAndSelectItems(full PIDL, cidl=0)` 打开父目录并选择项目；不拼接 explorer/cmd/PowerShell 参数。只平衡本次成功的 COM 初始化并始终释放 PIDL。真实 Windows 冒烟已确认 API 返回成功且 Explorer 精确选中目标。
...
### DEC-051：关联应用打开以 Restricted Zone 临时副本和三阶段 STA job 提交

- **状态：** 已接受
- **日期：** 2026-08-04
- **背景：** `DEC-048/049/050` 已提供 opaque `.cache`、pinned 完整性复验、最终授权、受控目录定位和进程内图片查看，但阶段 9 还要求显式“打开文件”。直接把 `.cache`、原名副本或任意路径交给系统关联会丢失扩展语义、MOTW、配额和撤权生命周期；在普通 async continuation 上创建 apartment COM 或在最终 commit 后才进行可失败 enqueue，又可能阻塞 UI 或让已授权动作静默丢失。
- **决策：** open copy 严格位于 `Path.GetTempPath()` 下的 RelayCove/account scope，根、scope 和文件拒绝 reparse point。完整 Windows leaf 先拒绝路径/ADS/非法字符、尾点/尾空格；terminal extension 的原始字符必须先是 1–16 位 ASCII 字母或数字，之后才规范为 lowercase，避免 Unicode case-folding 混入 ASCII。物理名使用 128-bit 随机 basename 和 `CreateNew`。不维护易过期的危险扩展 denylist，所有语法有效类型统一交给默认 Restricted Zone Attachment Manager 与企业策略。
- **决策：** 从既有 pinned pathless stream 复制后，以实际 EOF、长度和 SHA-256 复验主数据，并写入、回读 `Zone.Identifier` 的 `ZoneId=4`。因为副本确实位于系统 temp，遵循 Microsoft 临时附件范式不调用 `Save`；若未来迁移到非 temp，必须重新冻结 `Save`、杀毒修改/删除后的二次主数据验证。每账户 open store 独立使用最多 1 GiB **主数据流逻辑字节**和 64 项的 byte/count reservation，同附件 single-flight；不将此预算表述为覆盖 ADS 或实际磁盘分配的物理上限。只扫除严格托管 orphan，active lease 跨 runtime generation 不得被误删。
- **决策：** 应用生命周期拥有一个串行专用 STA worker；`CoInitializeEx(STA)`、`CoCreateInstance`、全部 `IAttachmentExecute` 调用和 Release 都在同一线程。只设置固定 client GUID/title 与 `SetLocalPath`，不设置 source/referrer/file name，不传 verb/argument/working directory；调用 `CheckPolicy` 与 `Execute(ownerHwnd, null, &process)`，无 `Process.Start` fallback。默认 null source 保持 Restricted Zone并避免把内部 server URL 交给 handler。返回的 process handle 仅按所有权立即 Close，不作为 cleanup 或查看器生命周期信号。
- **决策：** commit 前先建立不可变 job、cleanup lease 并由 worker 接管。三阶段边界固定为：Prepare 完成 `IAttachmentExecute` 配置与 policy check；最终 SQLite/access/UI commit 只把 pending job 置为 committed，并等待 STA 已切为 foreground 的 acknowledgement，此时仍不调用 COM `Execute`；只有整个 `ConfirmDownloadedAttachmentAsync` 调用已返回或抛出、SQLite transaction、local-cache operation gate、coordinator `CommitGate` 与 shell `stateGate` 全部退出后，协调器才发出一次 `ExecuteRelease`，STA 随后进行恰好一次 `Execute` 尝试。commit 前取消、撤权、selection/context 变化或任一验证失败 abort 且零 Execute；commit 后的撤权/logout 只能标记 purge pending，不能取消已授权 job。`CompleteLaunch` 只能发生在 `Execute` 返回、process handle 已立即 Close、COM 已 Release 且 completion 已发布之后。cleanup 失败保留 pending，并只在后续 revoke/logout/dispose/recovery cleanup 触发时重试；不声称后台任务或重试次数有界。正常关闭时 committed STA job 从 foreground acknowledgement 开始，覆盖等待 `ExecuteRelease`、`Execute`、handle/COM release、completion 与 retire，之后才恢复 background；`DisposeAsync` 只等待进入 `Execute` 或 terminal，不等待 `Execute` 返回。Windows trust/AV provider 在 `Execute` 中仍可删除或修改副本，进程强退也可能落在 commit 与实际 `Execute` 之间；二者是诚实记录的不可消除边界，由下次启动 orphan recovery 收敛。
...
- **来源：** 工程方案第 16、阶段 12/13；M4-02 Client ZIP/manifest 实证；`docs/ai/tasks/2026-08-04-m4-portable-updater-core.md`；production `96596d8..84b3b95`；两份 `1.0.0-rc.11` ZIP 字节一致、真实 rc.6→rc.11 自举替换、Fast/Full 1,514/1,514、model drift 与八项目漏洞审计；多路 Codex 复核关闭全部 P0/P1。Claude #82 challenge `c5c5ab8c` 实际 `claude-sonnet-5` / High，答案已读取并支持 portable ZIP/M4-03→M4-04 分层；其 stale lock/journal recovery 意见已修正，精确自举残留清理进入 M4-04。

### DEC-056：M4-04 以 exact 只读快照托管、受控客户端下载与显式交接完成内部 RC 更新闭环

- **状态：** 已接受
- **日期：** 2026-08-04
- **背景：** `DEC-055` 已冻结内部 `portable-zip`、外部自举 Updater 与离线替换恢复，但尚未规定 Server 如何只读暴露当前 exact manifest/artifact，也未冻结 Client 从不可信网络响应到 mandatory gate 和退出交接的提交边界。直接缓存打开的文件 stream 会把文件句柄、替换/删除和响应生命周期耦合；只按 URL 或旧 manifest 取文件会产生 manifest/artifact TOCTOU；下载时把未校验内容发布为成品、允许 HTTPS 降级或在 mandatory 的迟到响应中回退到可继续登录，都会破坏更新闭环。
- **决策：** Server 只匿名暴露配置指向、经 Shared 逐请求重新解析和验证的当前 manifest，及该 manifest 中唯一的 exact artifact；不提供目录浏览、任意文件名、写入或历史 artifact。可缓存 validated manifest snapshot 与 artifact metadata，绝不缓存 stream；每次重验证前先失效缓存（invalidate-before-revalidate），再重新解析 manifest 和安全地解析 artifact 路径。无效、缺失、越界、替换失败或不再匹配的 manifest/artifact 一律 fail-closed。
- **决策：** Client 只接受 Shared 验证后的 HTTPS artifact；测试用 loopback 是唯一明确例外。manifest 响应上限 64 KiB，整个响应有 15 s 上限；正文流入账户数据之外的受控更新缓存 `.part`，只在 exact size 与 SHA-256 均匹配后原子发布。不得把 `.part`、不完整响应、重定向/非 HTTPS 响应或不匹配内容交给 Updater。optional 更新可稍后；mandatory/登录 preflight 以单调状态收敛，只有尚未知 endpoint 的检查失败可 fail-open，已知 mandatory 端点的失败不得恢复旧版主功能。
- **决策：** 登录 preflight、启动检查和手动检查以版本化/单调结果提交，迟到的较弱结果不能覆盖已知 mandatory 状态。更新交接只从固定 App 目录启动固定 `RelayCove.Updater.exe`，传递结构化当前/目标版本、archive size/hash、PID 与启动时间；成功启动后 Client 走既有显式 Exit 生命周期，不等待 Updater，并清理本次 updater handoff token/临时所有权。真实验证使用安全的 Kestrel 与 safe-probe/发布目录 smoke，不把探针暴露为生产任意文件或进程控制面。
