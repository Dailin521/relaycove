...
把已完成的可信附件下载核心接入 production WPF 消息列表：已确认的 Image/File 消息显示严格投影的附件名、大小和本地状态；用户可以显式下载、查看真实接收进度、取消自己发起的 flight、失败后重试，并在每次重新验证物理内容与当前授权后对已下载文件执行受控本地访问。所有 UI 回调都必须抵御会话切换、撤权、账户重建和 A→B→A 的迟到结果，且不得把本地路径、URL、hash 或 ID 暴露给可渲染字段、presentation `ToString()`、日志或自动化名称；内部状态仍可持有 exact identity 用于授权和竞态门。
...
- `已验证`：Codex 三路只读探索已完成 WPF、竞态、UIA 与 Windows shell 边界映射；本机只读 Claude Opus/XHigh 后台任务 `a28c24d7` 的关键安全 challenge 答案已返回并读取，detached session 仍保持 idle、未取消；实际 `claude-opus-5`、约 26 分钟、CLI 显示费用 `$20.07`。其成立的 exact download membership 与 Ready 状态门已修正；STA/cidl=0 质疑由 Microsoft 官方契约、Codex 独立复核和真实 Explorer 冒烟反证后未采纳。
...
- [x] confirmed 附件按协议顺序显示安全文件名、大小和持久 Downloaded 状态；Text/System/pending 不产生伪附件动作，可渲染字段、presentation `ToString()`、UIA 与日志不暴露路径、URL、hash 或 ID。
- [x] 每附件下载进度单调；用户只可取消自己发起的 flight；Canceled/失败可重试，Completed/AlreadyDownloaded 才进入已下载状态；quota/auth/revoke/protocol/local/remote 状态有明确且不泄密的文案。
- [x] A→B→A、snapshot refresh、recycling、旧 progress/result、撤权/注销/退出均不能污染当前行；同一消息多附件互不串状态，焦点与 live region 可用且不过度播报。
- [x] 本地访问必须从 identity 重新读取/验证 cache 与最终授权；绝对路径、遍历、ADS、`.part`、错 ID/hash、缺失/损坏/reparse、DB 变化或撤权均零 shell 调用。
- [x] 定向、STA WPF、Fast、最终 Full、Codex reviewer、关键 Claude challenge 答案读取、model drift、依赖漏洞、脱敏与空白检查完成；真实登录附件视觉/Narrator 与 VPS 保持 `未验证` 到 M5。
...
- production 提交 `c7dea98`–`6a92064` 已把 confirmed Image/File 附件严格投影到 WPF：协议顺序、安全文件名/大小、持久 Downloaded 状态、真实接收进度、owned cancel、失败重试与可访问 live region 均接入现有虚拟化消息行。所有异步结果以当前 runtime、Ready selection、消息/附件 exact membership、context version、state/flight object identity 共同提交，覆盖切换、refresh、撤权与 A→B→A。
- “在文件夹中显示”只接受 attachment identity。cache 重新读取 SQLite，验证严格托管 scope/path、非 reparse、长度与完整 SHA-256，以只读 pinned handle 固定内容，再在短事务内复核完整记录和当前授权；提交后释放 state/cache/SQLite 锁才调用 Windows Shell。撤权、选择变化、数据库变化或 runtime 终止在提交前均为零 shell，提交后的外部 Explorer 启动作为线性化结果不伪装为可撤回。
- Windows interop 使用 `CoInitializeEx`、`SHParseDisplayName` 与官方支持的 `SHOpenFolderAndSelectItems(full PIDL, cidl=0)`，不构造 explorer/cmd/PowerShell 命令字符串。真实 Windows 冒烟返回 `init=1 parse=0 reveal=0`，UI Automation 确认 Explorer 在仓库目录选中 1 个 `README.md`。
- 两路 Codex reviewer 最终均为 PASS、无 P0–P2；Windows interop 独立复核无 P0–P2。Claude #76 的两项成立意见已修正，两个不成立的 P0 由官方文档、Codex 复核与实机结果裁定不采纳。
- 最终验证：附件/MessageListPresenter/AccountShell 定向 338/338；Release Full 1,211/1,211（Shared 39、Server 255、Client 916、Updater 1），0 警告、0 错误；EF model drift 无变化，八项目漏洞审计无易受攻击包，format、敏感日志检索与 `git diff --check` 通过。
- Full 曾两次命中既有通知测试的观察点竞态：fake 在原生注销开始时递增计数，而 host 在注销返回后才清 uncertainty。测试提交 `9e9d409` 改为等待 `TryStart()` 的真实可恢复行为，生产通知代码不变；最终版本定向 21/21 且 Full 通过。
- 明确限制：直接打开 opaque `.cache` 仍需另一个安全切片冻结文件关联、MOTW/Attachment Manager、扩展名与临时副本生命周期；本任务只显示所在文件夹。图片缩略图/有界原图查看进入下一切片；真实登录附件视觉、Narrator、VPS 与双客户端保留到 M5 Gate。
