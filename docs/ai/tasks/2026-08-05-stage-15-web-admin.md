...
- [x] `/relaycove/admin/` 能登录并完成现有全部基础管理功能，普通用户与未登录用户不能进入。
- [x] 浏览器只持有受限 Cookie；Cookie 不能访问管理 API，Bearer 不能代替网页登录；Cookie flags、Path 与重定向在子路径正确。
- [x] 缺少/伪造 antiforgery 的写请求失败；网页登录受限流；禁用、退役、改密或 token 代际变化后旧网页会话立即失效。
- [x] 用户、频道/成员和上传设置至少各有一条真实页面写操作回归；状态页不泄露额外敏感信息。
- [x] Fast、Full、发布打包、格式/空白与独立认证安全复核通过；香港 VPS HTTPS 子路径完成浏览器实测。
...
`已完成` — Server 网页管理面已部署香港 VPS，并完成真实浏览器登录、管理写入、公网接口、服务重启会话保持及普通账号登录验证。
...
| `已验证` | 两字符账号兼容与 migration | Fast/Full 1,613/1,613；旧库升级保留数据、`lq` 可写入、单字符仍拒绝；独立复审无 P0/P1。首次 Full 仅遇到一项既有 Windows 通知 2 秒时序偶发失败，精确原样重跑与随后完整 Full 均全绿 |
| `已验证` | Linux Server `1.0.0-rc.17` | `111045692` bytes，SHA-256 `23c2013852202cbbe592245ae4b73fa67dd7d474128b2885a7e5638efaf3891b`，本机 verifier 与 VPS 上传哈希一致 |
| `已验证` | 香港 VPS `/relaycove/admin/` | 公网登录页 200、后台未登录 302、更新接口 200、Hub 未授权 401；真实管理员登录、创建 `lq`/`dal`、两账号 API 登录与注销、服务重启后 Cookie 会话保持均通过 |
| `已验证` | 生产备份与数据库 | `/var/backups/relaycove/20260805T035502Z-pre-rc17` 全文件哈希通过；migration 应用后 `quick_check=ok`、外键违规 0、最新 migration 正确 |
...
- 运维记录：rc.17 首次启动因继承配置被误设为 `root:root` 而拒绝读取；在未成功提供新服务前修正为既有 `root:relaycove 0640`，随后启动、重启与公网验收均通过，迁移前备份始终保留。

### 下一步

- owner 可立即使用网页后台；若确认不再需要 Windows 管理入口，再以独立小切片移除旧 overlay。公开 main/Tag/Release 与严格第二 Windows UI 矩阵仍为可选后续。
