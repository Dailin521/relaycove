...
- `待参考`：按项目规则仅尝试一次 Claude XHigh 窄审查，最长 60 秒；无结论不重试、不阻塞 Codex。
...
- [ ] 任意极大 ID、跨会话 ID、空/非正 ID 稳定 400；未知/删除/撤权/禁用用户稳定 403，且权限检查先于目标消息信息暴露。
...
- 冻结 read-through 请求/确认及 Public 个人状态行语义，完成一次 Claude 参考后实现。
