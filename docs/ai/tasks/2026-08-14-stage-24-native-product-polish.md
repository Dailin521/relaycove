...
- Composer uses native Windows pointer capture and cleanup, clamps 128–300 DIP, retains keyboard adjustment and does not overwrite a larger user height when attachments appear. The 128 DIP minimum keeps a visible 64 DIP editor row and toolbar; the transparent 16 DIP resize band overlaps the Composer top border without a separate visual handle.
