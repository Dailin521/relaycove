...
- [ ] URI/token 边界正确：只接受无 user-info/query/fragment 的绝对 HTTP(S) base URI，连接固定相对 `hubs/chat`；token provider 在连接请求时动态读取且 token 不进入日志/状态对象。
- [ ] 真实认证连接能把完整 `MessageDto` 与正确撤权 conversation ID 交给 sink；撤权事件先于随后到达的同会话消息完成处理，sink 异常不会终止事件消费或泄漏敏感载荷。
- [ ] Start/Stop、初始 401/不可用、Reconnecting/Reconnected/Closed 映射为冻结状态；重复并发生命周期调用不会创建多连接或死锁，Dispose 后不可重启。
...
- 固定客户端连接决策后实现最小生产组件和真实内存 Hub 测试。
