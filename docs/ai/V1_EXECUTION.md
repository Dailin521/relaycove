...
NextAction: 仅快进合入已验收的搜索 API，随后接客户端 current/global 搜索与 Around 跳转高亮
...
| M3 | `running` | 权限化中文/附件名搜索 Shared+Server API 已由 `4c4edba` 完成全部门禁，待仅快进合入 | 完成客户端 current/global 搜索、Around 跳转与高亮的 Beta 证据 |
