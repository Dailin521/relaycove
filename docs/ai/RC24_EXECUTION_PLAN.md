...
- **计划状态：** `自动化范围已完成；S5 经 owner 决定跳过且保持未验证，真实 S6/S7 不构成当前 AI 验证结论且仍受授权 Gate 约束`
- **目标版本：** `1.0.0-rc.24`
...
### 15.0 Owner 测试边界（2026-08-08）

- owner 指定本轮采用全程 AI 自动化测试，Windows UI 使用 WPF 内置 `RenderTargetBitmap` 快照并由内部图片工具复核。
- 三档快照及相应可用性回归已完成；W01–W15 的真实双账号、托盘、断网、旧 Toast 和安装态升级场景仍为 `未验证`，不得表述为通过。
...
> Owner 已选择当前 AI/WPF 验证范围；以上真实 Windows 人工矩阵项目保持未验证，不作为本轮自动化通过结果。
